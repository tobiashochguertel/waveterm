...
[wavetermdev/waveterm](https://github.com/wavetermdev/waveterm) and evaluates
...
as `.github/UPSTREAM-PRS.md`. A companion CLI tool (`waveterm-prs`) lets you
...
| Viewer CLI | `waveterm-prs` (in `~/bin/`) | Fetches and displays the catalog locally |
...
| `origin` | Links to `https://github.com/wavetermdev/waveterm/blob/main/<filename>` (the unmodified upstream file) |
| `pr-diff` | Links to the specific diff anchor in the PR: `https://github.com/wavetermdev/waveterm/pull/<N>/files#diff-<sha256>` |
...
`[#NNN](https://github.com/wavetermdev/waveterm/pull/NNN)` (clickable links).
...
```bash
# From the CLI
gh workflow run update-upstream-prs.yml --repo tobiashochguertel/waveterm --ref main

# Check the run status
gh run list --repo tobiashochguertel/waveterm --workflow update-upstream-prs.yml --limit 3
```
...
## Viewer CLI (`waveterm-prs`)

A PEP 723 script installed via `scriptmgr` to `~/bin/waveterm-prs`. Fetches the
catalog from GitHub and displays it in the terminal, browser, or editor.

### Installation

The script lives at `~/dotfiles/tools/pep723-scripts/waveterm-prs.py` and is
...
```bash
waveterm-prs                         # show in terminal (rich markdown tables)
waveterm-prs --browser               # open GitHub-rendered page in browser
waveterm-prs --editor                # open raw markdown in $EDITOR
waveterm-prs --raw                   # print raw markdown to stdout (for piping)
waveterm-prs --regenerate            # regenerate locally, then show in terminal
waveterm-prs --regenerate --browser  # regenerate, then open in browser
waveterm-prs --branch dev.patch      # fetch from dev.patch instead of main
waveterm-prs --repo user/repo        # fetch from a different fork
```
...
| `--regenerate` | `-r` | Run the generator script locally before fetching. Requires the WaveTerm fork at `~/work/dev/tools/waveterm` |
| `--repo` | | GitHub repo in `owner/name` format (default: `tobiashochguertel/waveterm`) |
...
|----------|---------|-------------|
| `WAVETERM_PRS_REPO` | `tobiashochguertel/waveterm` | Default repo |
| `WAVETERM_PRS_BRANCH` | `main` | Default branch |
...
(`uv run --script .github/scripts/generate_upstream_prs.py`) in the WaveTerm
...
```bash
waveterm-prs --branch dev.patch
waveterm-prs --branch dev.patch --browser
```
